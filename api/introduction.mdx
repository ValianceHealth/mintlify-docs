...
- **PDPA (Malaysia).** The Personal Data Protection Act 2010 restricts cross-border transfer of personal data: disclosures must go to destinations you can name and justify, and processors carry binding security duties. A generic LLM endpoint satisfies neither.
- **HIPAA (US).** Protected Health Information may only reach vendors under a Business Associate Agreement with strict safeguards. De-identified data falls outside that restriction.
...
flowchart TB
    ...
    B -->|"Text with placeholders only"| C["PHI-approved LLM\nAPAC-bounded hosting"]
